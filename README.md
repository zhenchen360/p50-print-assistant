...
USB 备用路径用于蓝牙不可用、需要临时走 Windows 打印队列的情况。使用前需要先安装 P50S Windows 打印驱动，并在系统里出现 `P50 Printer` 之类的打印机名称。程序会先生成与预览一致的标签点阵，再按 USB 驱动方向做补偿后交给 Windows 打印驱动，并保持驱动的“绘制边框”关闭。P50 Windows 驱动在 40 mm 宽 GDI 自定义页面上会出现固定边界裁剪，程序会对 40 mm 标签使用 39 mm 的 USB 安全宽度避开该边界；最终走纸、边距和定位效果仍取决于驱动与系统打印设置。
