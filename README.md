...
USB 备用路径用于蓝牙不可用、需要临时走 Windows 打印队列的情况。使用前需要先安装 P50S Windows 打印驱动，并在系统里出现 `P50 Printer` 之类的打印机名称。程序会先生成与预览一致的标签点阵，再交给 Windows 打印驱动；最终走纸、边距和定位效果仍取决于驱动与系统打印设置。
